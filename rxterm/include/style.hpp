...
constexpr Font getFontStyle(Font f = Font::None) { return f; }

template <class X, class... Xs>
constexpr Font getFontStyle(X x, Xs... xs) {
    return Font(((int)getFontStyle(toFont(x))) |
                ((int)getFontStyle(toFont(xs)...)));
}

bool has(Font x, Font y) { return (((int)x) & ((int)y)); }
...
struct Style {
    ...
    string defaultMod() const { return has(font, Font::Default) ? "0" : ""; }

    string boldMod() const { return has(font, Font::Bold) ? "1" : ""; }

    string underlineMod() const {
        return has(font, Font::Underline) ? "4" : "";
    }

    string faintMod() const { return has(font, Font::Faint) ? "2" : ""; }

    string italicMod() const { return has(font, Font::Italic) ? "3" : ""; }

    string hiddenMod() const { return has(font, Font::Hidden) ? "8" : ""; }

    string crossedMod() const { return has(font, Font::Crossed) ? "9" : ""; }
    ...
    std::string toString() const {
        return computeMod(defaultMod(), boldMod(), underlineMod(), faintMod(),
                          italicMod(), hiddenMod(), crossedMod(), bgMod(),
                          fgMod());
    }
};
...
Style diff(Style const& a, Style const& b = Style::None()) {
    ...
    return Style{(keepBG && !reset) ? Color::Inherit : b.bg,
                 (keepFG && !reset) ? FontColor::Inherit : b.fg,
                 (keepFont && !reset) ? Font::Inherit
                                      : (Font)((r & ((l & r) ^ r)) | reset)};
}
